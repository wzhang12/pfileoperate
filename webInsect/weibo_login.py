...
def do_login(username,pwd,cookie_file):
    ...
    http_headers = {'User-Agent':'Mozilla/5.0 (X11; Linux i686; rv:8.0) Gecko/20100101 Firefox/8.0','Accept-Language':'zh-CN,zh;q=0.8,en;q=0.6'}
...
def relogin(username,password,cookie_file):
	remove_cookie(cookie_file)
	return login(username,password,cookie_file)

def remove_cookie(cookie_file):
	try:
...
if __name__ == '__main__':
    ...
    if login(username, pwd, cookie_file):
        ...
        # file=open("firsta.html","w")
        # file.write(kaifu_page)
        # file.closed
    else:
